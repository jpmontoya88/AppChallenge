...
/*
func api_cast_request<T>(with url: URL)-> AnyPublisher<[T], enum_error_type> where T:Decodable{
    ...
    return URLSession.shared.dataTaskPublisher(for: request)
        ...
        .flatMap(maxPublishers: .max(1)){ response in
            decode_cast(data: response.data)
        }
        .eraseToAnyPublisher()
}

func decode_cast<T>(data: Data)->AnyPublisher<[T], enum_error_type> where T:Decodable{
    let decoder = JSONDecoder()
    return Just(data)
        .decode(type: [T].self, decoder: decoder)
        .mapError{ error in
